...
# input_file <- 'cfDNA_fragmentomics/results/end_motif_220809/all_selected_features/glmnet/end_motif_6.csv'
# output_dir <- 'svm'
...
svmGrid <- expand.grid(sigma = seq(0, 0.3, 0.05), C = seq(0.5, 5.5, 0.5))
...
pred.tbl <- model_svm$pred %>% filter(C==model_svm$best$C, sigma==model_svm$best$sigma) %>% group_by(rowIndex) %>% dplyr::summarize(obs=obs[1], Cancer=mean(Cancer))
