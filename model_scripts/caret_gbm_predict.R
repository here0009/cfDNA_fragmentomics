...
# raw_trainning <- input_table[train_idx,]
# preProcValues <- preProcess(raw_trainning, method = c("center", "scale"))
# trainning <- predict(preProcValues, raw_trainning)
...
# gbmGrid <- data.frame(n.trees=200,interaction.depth=1,shrinkage=0.2,n.minobsinnode=5)
gbmGrid <- expand.grid(interaction.depth = c(1,2,3,4), n.trees=(1:5)*50, shrinkage=c(0.1,0.2,0.3), n.minobsinnode=c(5,10,15))
...
dev.off()
# save model
saveRDS(model_gbm, file.path(output_dir, "model.rds"))
# model_gbm <- readRDS('model.rds')
# model_gbm <- readRDS(file.path(output_dir, "end_6_models_list.rds"))
# check the performance of model of trainning data
# model_gbm$pred %>% filter(model)
...
test_pred <- predict(model_gbm, test[,-dim(test)[2]]) # remove last column of test
# model_gbm$results %>% filter(n.trees==model_gbm$best$n.trees, interaction.depth==model_gbm$best$interaction.depth, shrinkage==model_gbm$best$shrinkage, n.minobsinnode==model_gbm$best$n.minobsinnode)
